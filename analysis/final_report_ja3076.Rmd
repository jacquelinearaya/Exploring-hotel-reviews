---
title: "Is your country saying something about how you review products?"
...
Reviews are understood as qualitative - written reviews- or quantitative - ratings - feedback leave by customers who paid for a product/service. Usually online sites ask for customers to leave a review, after the purchase is done, in their website with some sort of system review, mixing a qualitative aspect such as text written by a person or a quantitative one by assigning a score, number of stars, selecting sad/happy emoji faces, select a position in a score bar, etc. 
...
In this context, I thought it would be interesting to study product/service reviews from a perspective of customer's nationality and explore whether there is a cultural influence given by our country of origin in reviewing products or services, claiming that, generally speaking some cultures can be more inquisitive, while others more laid-back or less-more educated, less-more materialist among a big number of differences.

One service where a lot of people from different countries converge to is hotels, and thanks to companies for online booking, like Expedia, Trivago, Priceline, Booking, Hotels.com and many others, we can search, compare and pick among thousand of hotels. These sites rely heavily on past guest reviews to rank and list hotels for any search the user may do, including features like price, location, amenities, stars and so on. All of these sites have different review systems, but generally, they focus on two components: a written review and a score of some sort. These reviews can also show some user data and are often asked after the guest's stay has ended. 

Having in mind the goal of this analysis, I picked an online booking site called Booking.com. This site displays on every hotel review leaved by a guest its nationality, so it's a good proxy to study whether these other factors tend to reveal differences in reviews. And if so, do people from a common cultures tend to score lower or higher consistenly? What hotel features do tourists from different places care for more? Do hotels with an international profile tend to get better scores from local or international tourists? These are the types of question I'm looking to answer with the data collected from Booking.com.



## Description of the data source

Booking.com is an online booking site for hotels, flights, cars and other touristic services based in Amsterdam. 
Booking.com has a very nice feature in their review system, not only you can view written reviews in many different languages, but they also display the nationality of the user that write the review:

![User posts for a hotel in Booking.com](./post_example.png)


Each hotel has two components of what comprises their review: an overall score of the hotel and a list of all guest reviews, which are breakdown into a written part and a score (as in the previuos picture).

![Overall hotel score in Booking.com](./overall_score.png)

I will focus my analysis in the quantitative part of the review, collecting the overall score and its breakdown for each chosen hotel and the individual score given by each guest. The overall score is calculated with at least 5 reviews and it's based on the last 24 months reviews. According to Booking.com:

> "Guests can evaluate 6 separate categories with 4 smiley or sad faces, each face counting for a score of 2.5.
>When we calculate your average score, we add up the scores for Cleanliness, Comfort, Location, Facilities, Staff, and Value, and we divide this number by 6 (i.e. the number of individual categories)
>So each review that comes in won’t make an immediate impact on your property average score.
>When guests don’t give a score for a given category, that category score defaults to zero and won’t be used when calculating your average score."

Along with those mentioned categories there is an extra new one: "WiFi".

The selection of hotels I chose to collect data is totally subjective and it's based on the believe that hotels in United States receive a variety of tourist from around the world. I picked the top 10 most visited cities in United States according to the U.S. Department of Commerce, ITA, National Travel & Tourism Office (Survey of International Air Travelers) in 2017.
...
I chose to stick with the default of the site, which is the "Top Picks" and collect basic data of all the hotels in that list. Afterwards, for each collected hotel, I got the overall score and the full list of guest scores for all languages.
The collection of data was made with a self-coded web scraping script written in Python [Link to Python files](https://github.com/jacquelinearaya/StatsGR5293_project/tree/master/data/raw) adhoc to this task and was stored in several json files. As the web scraping process was done in two steps, it was natural to come up with two different, but linked, datasets: one for hotels and their basic information along with the overall score, and a second one with the guest's scores for each hotel.
...
These files, along with the raw data can be found in the following GitHub Repository: [Github Repo](https://github.com/jacquelinearaya/StatsGR5293_project).

```{r read_files_hotels}

setwd("~/Documents/DSI_certificate/gr5293StatisticalGraphics/final_project")
...
The collected data with the python script was dump into json files by city given the structured nature of it. Next, the preprocessing was done by a second python script in which it parsed the json files into a dataframe format, and then into CSV files [Link to csv files](https://github.com/jacquelinearaya/StatsGR5293_project/tree/master/data/tidy).

Once the CSV files were ready to import into R, the following transformations were made:

* From Hotel Dataframe:
  * Variable "city" was coded as factor
  * Variable "reviews" was coded as factor
  
* From Guest's reviews Dataframe:
  * Variable "city" was coded as factor
  * Variable "review_country" was coded as factor
  * Variable"review_date" was coded as Date with format "YYYY-MM-DD"
...
A simple sum of columns let us know the extent of missing value per variable. From the total number of observations, 6,738, we can note that 3,875 values are missing for the score aspects of each hotel. This fact may be due to lack of guest's reviews to calculate an overall score explained by a number of reason such that an average experience may not be worth the time to leave a review or the hotel itself doesn't encourage guests to leave feedback, but certainly it's a point to investigate further.

We can see a breakdown of these missing values per city in the tile plot. Is easy to see that New York City and Miami are the cities with a more complete profile of data, as shown below:
...
Then, we have a number of observations with missing stars, specifically 2,775, that represents close to 41% of the hotels. 


Since the goal of this study is to explore differences within countries, we can work with all reviews of hotels with complete data (2,863) that accounts for more than 1 million guests reviews in total.
...
# tidyrev <- posts %>% 
#     select(-c(hotel_id,review_id)) %>%
#     gather(key, value, -city) %>% 
#     mutate(missing = ifelse(is.na(value), "yes", "no"), key=as.factor(key))
# 
# ggplot(tidyrev, aes(x = fct_relevel(key,sort_pvars), y = fct_rev(city), fill = missing)) +
#   geom_tile(color = "white") + 
#   scale_fill_viridis_d() + # discrete scale
#   ylab("City") +
#   xlab("Variable")+
...
# 
# ggplot(tidystars, aes(x = key)) +
#   geom_bar(aes(yes)) + 
#   facet_grid(cols =vars(hotel_stars))
#   ylab("Stars") +
#   xlab("Variable")+
#   theme_bw(16) +
#   theme(axis.text.x=element_text(angle=60,hjust=1)) 
...
```{r medcountry, fig.height=10, fig.width=7}
...
```{r sdcountry, fig.height=10, fig.width=7}
...
```{r countrystars, fig.height=8, fig.width=7}
...
# post_filterstars %>%
#   filter(review_country %in% n5000$review_country) %>%
#   ggplot(aes(mean, reorder(review_country,mean), color=hotel_stars)) +
#   geom_point() +
#   labs(x="Review Score mean", y="Country", title="Guest's review score mean by country\nwith more than 5,000 reviews") +
...
```{r countrystars2, fig.height=8, fig.width=7}
...
# post_filterstars %>%
#   filter((review_country %in% n5000$review_country) & hotel_stars==5) %>%
#   ggplot(aes(mean, reorder(review_country,mean))) +
#   geom_point(color="#F564E3") +
#   labs(x="Review Score mean", y="Country", title="Guest's review score mean by country for \n5 star hotels with more than 5,000 reviews") +
#   scale_x_continuous(breaks = seq(7,10,0.1)) +
#   theme_bw(14) +
#    theme(axis.text.y = element_text(size = rel(.75)),
#     	axis.ticks.y = element_blank(),
#         axis.title.x = element_text(size = rel(.75)),
...
# 
# post_filterstars %>%
#   filter((review_country %in% n5000$review_country) & hotel_stars==4) %>%
#   ggplot(aes(mean, reorder(review_country,mean))) +
#   geom_point(color="#619CFF") +
#   labs(x="Review Score mean", y="Country", title="Guest's review score mean by country for \n4 star hotels with more than 5,000 reviews") +
#   scale_x_continuous(breaks = seq(7,10,0.1)) +
#   theme_bw(14) +
#    theme(axis.text.y = element_text(size = rel(.75)),
#     	axis.ticks.y = element_blank(),
...
# 
# post_filterstars %>%
#   filter((review_country %in% n5000$review_country) & hotel_stars==3) %>%
#   ggplot(aes(mean, reorder(review_country,mean))) +
#   geom_point(color="#00BFC4") +
#   labs(x="Review Score mean", y="Country", title="Guest's review score mean by country for \n3 star hotels with more than 5,000 reviews") +
#   scale_x_continuous(breaks = seq(7,10,0.1)) +
#   theme_bw(14) +
#    theme(axis.text.y = element_text(size = rel(.75)),
#     	axis.ticks.y = element_blank(),
...
# 
# post_filterstars %>%
#   filter((review_country %in% n5000$review_country) & hotel_stars==2) %>%
#   ggplot(aes(mean, reorder(review_country,mean))) +
#   geom_point(color="#00BA38") +
#   labs(x="Review Score mean", y="Country", title="Guest's review score mean by country for \n2 star hotels with more than 5,000 reviews") +
#   scale_x_continuous(breaks = seq(5,10,0.1)) +
#   theme_bw(14) +
#    theme(axis.text.y = element_text(size = rel(.75)),
#     	axis.ticks.y = element_blank(),
#         axis.title.x = element_text(size = rel(.75)),
...
# 
# post_filterstars %>%
#   filter((review_country %in% n5000$review_country) & hotel_stars==1) %>%
#   ggplot(aes(mean, reorder(review_country,mean))) +
#   geom_point(color="#B79F00") +
#   labs(x="Review Score mean", y="Country", title="Guest's review score mean by country for \n1 star hotels with more than 5,000 reviews") +
#   scale_x_continuous(breaks = seq(5,10,0.1)) +
#   theme_bw(14) +
#    theme(axis.text.y = element_text(size = rel(.75)),
#     	axis.ticks.y = element_blank(),
...
# post_filterstars %>%
#   filter((review_country %in% n5000$review_country) & hotel_stars==0) %>%
#   ggplot(aes(mean, reorder(review_country,mean))) +
#   geom_point(color="#F8766D") +
#   labs(x="Review Score mean", y="Country", title="Guest's review score mean by country for \n0 star hotels with more than 5,000 reviews") +
#   scale_x_continuous(breaks = seq(7,10,0.1)) +
#   theme_bw(14) +
#    theme(axis.text.y = element_text(size = rel(.75)),
#     	axis.ticks.y = element_blank(),
...
```{r facet, fig.height=15, fig.width=7,eval = FALSE, include=FALSE}

n10000 =posts %>% 
  select(review_country) %>%
  count(review_country)%>% filter(n>10000)

post_filterstars %>%
  filter(review_country %in% n10000$review_country) %>%
  ggplot(aes(mean, reorder(review_country,mean), color=hotel_stars)) +
  geom_point() +
  facet_grid(hotel_stars~. , scales="free") +
  labs(x="Review Score mean", y="Country", title="Guest's review score mean by country\nwith more than 5,000 reviews") +
  scale_x_continuous(breaks = seq(4,10,0.5)) +
  theme_bw(14) +
   theme(axis.text.y = element_text(size = rel(.75)),
...
posts %>% 
   ...
   ggplot(aes(x=fct_reorder(review_continent, review_score, fun=median), y=review_score)) +
    geom_boxplot() +
  coord_flip()+
  xlab("Continent")+
  ylab("Review Score")+
...
posts %>% 
  select( review_score, review_continent) %>%
  filter(!is.na(review_continent) & review_continent!="Antarctica") %>%
  ggplot(aes(review_score, fct_reorder(review_continent, review_score, fun=median))) +
...
```{r contcity, fig.height=8, fig.width=7}
